...
class SonderwunschAussenanlagenPreisBerechnungTest {
	...
	*/
    ...
    public double berechneUndZeigePreisSonderwuensche(){
  		...
  		if (state_ABSTELL_TERRASSE_EG)	preis += Sw.ABSTELL_TERRASSE_EG.preis;
    	if (state_VEA_MARKISE_EG)		preis += Sw.VEA_MARKISE_EG.preis;
    	if (state_VEA_MARKISE_DG) 		preis += Sw.VEA_MARKISE_DG.preis;
    	if (state_E_MARKISE_EG)			preis += Sw.E_MARKISE_EG.preis;
    	if (state_E_MARKISE_DG)			preis += Sw.E_MARKISE_DG.preis;
    	if (state_EA_GARAGENTOR)		preis += Sw.EA_GARAGENTOR.preis;
    	if (state_ST_GARAGENTOR)		preis += Sw.ST_GARAGENTOR.preis;
    ...
    /*
    @Test
	public void testAlleSonderwuenscheAusgewaehltPreis() throws Exception {
		state_ABSTELL_TERRASSE_EG = true;
		state_VEA_MARKISE_EG = true;
		state_VEA_MARKISE_DG = true;
		state_E_MARKISE_EG = true;
		...
		if (expected.length == actual.length) {
			...
			for(int i = 0; i < expected.length; i++) {
				System.out.println("Expected--> " + expected[i] + "\t" + actual[i] + " <--Actual");
			}
		}
	...
	*/
	...
	/*
	public int[] checkboxenZuIntArray() {
