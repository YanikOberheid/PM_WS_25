...
class FensterAussentuerenPreisTest {
	...
	void init() throws Exception {
		con = business.kunde.DatabaseConnection.getInstance().getConnection();
	...
	void testPreisberechnungFensterAussentueren() throws Exception {
		...
		java.util.Map<Integer, Integer> preise = (java.util.Map<Integer, Integer>) f.get(null); // static field

		int actual = java.util.Arrays.stream(ids).map(id -> preise.getOrDefault(id, 0)).sum();
	...
	private int summePreiseAusDb(int[] ids) throws Exception {
		String placeholders = String.join(",", Arrays.stream(ids).mapToObj(i -> "?").toArray(String[]::new));

		String sql = "SELECT COALESCE(SUM(Preis),0) FROM Sonderwunsch " + "WHERE idSonderwunsch IN (" + placeholders
				+ ")";
