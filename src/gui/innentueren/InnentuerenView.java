...
public class InnentuerenView extends BasisView {
	...
	// --- GUI-Elemente ---
	private Label lblKlarglasTuerInnen = new Label(Sw.KLARGLAS_TUER_INNEN.bes);
	...
	private Label lblMilchglasTuerInnen = new Label(Sw.MILCHGLAS_TUER_INNEN.bes);
	...
	private Label lblHolztuerGarage = new Label(Sw.HOLZTUER_GARAGE.bes);
	...
	protected void initKomponenten() {
		...
		getGridPaneSonderwunsch().add(getSpinKlarglasTuerInnen(), column++, row);
		row++;
		column = 0;
		...
		getGridPaneSonderwunsch().add(getSpinMilchglasTuerInnen(), column++, row);
		row++;
		column = 0;
		...
		getGridPaneSonderwunsch().add(getSpinHolztuerGarage(), column++, row);
		row++;
		column = 0;
	}
	...
	public void updateSwCheckboxen(int[] ausgewaehlteSw) {
		...
		/*
		 * final int ID_4_1_KLARGLAS = 18; // idSonderwunsch final int ID_4_2_MILCHGLAS
		 * = 19; // idSonderwunsch final int ID_4_3_GARAGE = 20; // idSonderwunsch
		 * 
		 * // UI-Reset sp41.getValueFactory().setValue(0);
		 * sp42.getValueFactory().setValue(0); cb43.setSelected(false);
		 * 
		 * if (ausgewaehlteSw == null) return;
		 * 
		 * // Markierungen anhand der gespeicherten IDs setzen for (int id :
		 * ausgewaehlteSw) { if (id == ID_4_1_KLARGLAS) {
		 * sp41.getValueFactory().setValue(1); } else if (id == ID_4_2_MILCHGLAS) {
		 * sp42.getValueFactory().setValue(1); } else if (id == ID_4_3_GARAGE) {
	...
	public boolean[] holeIsSelectedFuerCheckboxen() {
		...
		/*
		 * return new boolean[] { getAnzahl41() > 0, getAnzahl42() > 0, isAuswahl43() };
		 */
		return null;
	}
	...
	protected int[] checkboxenZuIntArray() {
		// DB-IDs für diese Kategorie (bei Bedarf auf eure echten IDs anpassen)
		/*
		 * final int ID_4_1_KLARGLAS = 18; final int ID_4_2_MILCHGLAS = 19; final int
		 * ID_4_3_GARAGE = 20;
		 * 
		 * java.util.ArrayList<Integer> ids = new java.util.ArrayList<>(); if
		 * (getAnzahl41() > 0) ids.add(ID_4_1_KLARGLAS); if (getAnzahl42() > 0)
		 * ids.add(ID_4_2_MILCHGLAS); if (isAuswahl43()) ids.add(ID_4_3_GARAGE); return
	...
	public void updateSwInView(int[][] ausgewaehlteSw) {
		// Alles zurücksetzen
		getSpinKlarglasTuerInnen().getValueFactory().setValue(0);
		getSpinMilchglasTuerInnen().getValueFactory().setValue(0);
		getSpinHolztuerGarage().getValueFactory().setValue(0);
		...
		for (int[] sw : ausgewaehlteSw) {
			...
			case KLARGLAS_TUER_INNEN:
				getSpinKlarglasTuerInnen().getValueFactory().setValue(sw[1]);
				break;
			case MILCHGLAS_TUER_INNEN:
				getSpinMilchglasTuerInnen().getValueFactory().setValue(sw[1]);
				break;
			case HOLZTUER_GARAGE:
				getSpinHolztuerGarage().getValueFactory().setValue(sw[1]);
				break;
			default:
				System.out.println("Unbekannte Sonderwunsch-ID zu Innentüren: " + sw[0]);
			}
		}
	}

	@Override
	public int[][] checkboxenZuAnzahlSonderwuensche() {
		Vector<int[]> v = new Vector<>();

		int anzahl = 0;

		if (getSpinKlarglasTuerInnen().getValue() > 0) {
			anzahl = getSpinKlarglasTuerInnen().getValue();
			v.add(new int[] { Sw.KLARGLAS_TUER_INNEN.id, anzahl });
		}
		if (getSpinMilchglasTuerInnen().getValue() > 0) {
			anzahl = getSpinMilchglasTuerInnen().getValue();
			v.add(new int[] { Sw.MILCHGLAS_TUER_INNEN.id, anzahl });
		}
		if (getSpinHolztuerGarage().getValue() > 0) {
			anzahl = getSpinHolztuerGarage().getValue();
	...
	public Spinner<Integer> getSpinKlarglasTuerInnen() {
		return spinKlarglasTuerInnen;
	}

	public void setSpinKlarglasTuerInnen(Spinner<Integer> spinKlarglasTuerInnen) {
		this.spinKlarglasTuerInnen = spinKlarglasTuerInnen;
	}

	public Spinner<Integer> getSpinMilchglasTuerInnen() {
		return spinMilchglasTuerInnen;
	}

	public void setSpinMilchglasTuerInnen(Spinner<Integer> spinMilchglasTuerInnen) {
		this.spinMilchglasTuerInnen = spinMilchglasTuerInnen;
	}

	public Spinner<Integer> getSpinHolztuerGarage() {
		return spinHolztuerGarage;
	}

	public void setSpinHolztuerGarage(Spinner<Integer> spinHolztuerGarage) {
		this.spinHolztuerGarage = spinHolztuerGarage;
	}

	/*
	 * @Override public int[][] spinnerZu2DIntArray() { return new int[][] {
