...
public final class FensterControl {
	...
	public void leseFensterSonderwuensche() {
		int[][] ausgewaehlteSw = kundeModel.gibAusgewaehlteSwMitAnzahlAusDb(SwKategorie.FENSTER_AUSSENTUEREN.id);
	...
	public void speichereSonderwuensche(int[] fensterSw) {
		...
		ausgewaehlteSw = Arrays.stream(ausgewaehlteSw).filter(sw -> sw < 300 || sw >= 400).toArray();
	...
	public void speichereSonderwuensche(int[] fensterSw, int[][] fensterSwMitAnzahl) {
		...
		try {
			kundeModel.speichereSonderwuenscheFuerKategorie(fensterSw, fensterSwMitAnzahl,
	...
	public void speichereSonderwuensche(int[][] fensterSwMitAnzahl) {
		...
		try {
			kundeModel.speichereSonderwuenscheFuerKategorie(fensterSwMitAnzahl, SwKategorie.FENSTER_AUSSENTUEREN.id);
		} catch (Exception e) {
	...
	@Deprecated
	public boolean pruefeKonstellationFensterAussentueren(int[] ausgewaehlteSw, int[][] ausgewaehlteSwMitAnzahl) {
		return true; // TODO
	}
	...
	public boolean pruefeKonstellationFensterAussentueren(int[][] ausgewaehlteSwMitAnzahl) {
		/*
		 * - Prüfe, ob ein Sw in ausgewaehlteSw und ausgewaehlteSwMitAnzahl doppelt
		 * vorkommt. - Hole mit KundeModel.holeAusgewaehlteSwAusDbOhneKategorie() oder
		 * KundeModel.holeAusgewaehlteSwMitAnzahlAusDbOhneKategorie() die Auswahl der
		 * anderen Kategorien. - Prüfe, ob neue Auswahl mit der alten vereinbar ist und
		 * die Anzahlen erlaubt.
		 */
		return true;
	}

	public static String erstelleCsvInhaltFenster(int[][] fensterSwMitAnzahl) {

		StringBuilder sb = new StringBuilder();
		sb.append("Sonderwunsch;Anzahl;Einzelpreis;Gesamtpreis\n");

		if (fensterSwMitAnzahl == null) {
			return sb.toString();
		}
		...
		for (int[] sw : fensterSwMitAnzahl) {
			...
			sb.append(sonderwunsch.bes).append(";").append(anzahl).append(";").append(einzelpreis).append(";")
	...
	public void exportiereSonderwuenscheAlsCsv(int[][] fensterSwMitAnzahl) {
		...
		try {
			if (fensterSwMitAnzahl == null || fensterSwMitAnzahl.length == 0) {
				System.out.println("Export: Keine Sonderwünsche zu Fenstern/Außentüren vorhanden.");
				return;
			}

			if (kundeModel.getKunde() == null) {
				System.out.println("Export fehlgeschlagen: Kein Kunde ausgewählt.");
				return;
			}
			...
			String csv = erstelleCsvInhaltFenster(fensterSwMitAnzahl);
			...
			e.printStackTrace();
			System.out.println("Export fehlgeschlagen: Die CSV-Datei konnte nicht erstellt werden.");
		}
	}
}
