...
public class FensterView extends BasisView {
	...
	protected void berechneUndZeigePreisSonderwuensche() {
		 // IDs wie beim Speichern einsammeln (301–309)
	    java.util.Vector<Integer> v = new java.util.Vector<>();

	    if (chckBxSchiebetuerEg.isSelected())  v.add(301);
	    if (chckBxSchiebetuerDg.isSelected())  v.add(302);
	    if (chckBxEinbruchschutz.isSelected()) v.add(303);

	    if (chckBxVorbereitungEg.isSelected()) v.add(304);
	    if (chckBxVorbereitungOg.isSelected()) v.add(305);
	    if (chckBxVorbereitungDg.isSelected()) v.add(306);

	    if (chckBxRollladenEg.isSelected())    v.add(307);
	    if (chckBxRollladenOg.isSelected())    v.add(308);
	    if (chckBxRollladenDg.isSelected())    v.add(309);
	    ...
	    // Preis im Control berechnen lassen
	    int betrag = fensterControl.berechnePreis(ids);

	    // kompakte, userfreundliche Anzeige
	    javafx.scene.control.Alert info = new javafx.scene.control.Alert(javafx.scene.control.Alert.AlertType.INFORMATION);
	    info.setTitle("Preisberechnung");
	    info.setHeaderText("Ausgewählte Sonderwünsche – Fenster/Außentüren");
	    info.setContentText(String.format("Gesamtpreis: %,d €", betrag));
	    info.showAndWait();
	}
