...
public class FensterView extends BasisView {
	...
	// GUI-Elemente
	private Label lblSchiebetuerEg = new Label(Sw.STUEREN_TERRASSE.bes);
	...
	private Label lblSchiebetuerDg = new Label(Sw.STUEREN_DACHTERRASSE.bes);
	...
	private Label lblEinbruchschutz = new Label(Sw.EBS_HAUSTUER.bes);
	...
	private Label lblVorbereitungEg = new Label(Sw.VEAR_EG.bes);
	...
	private Label lblVorbereitungOg = new Label(Sw.VEAR_OG.bes);
	...
	private Label lblVorbereitungDg = new Label(Sw.VEAR_DG.bes);
	...
	private Label lblRollladenEg = new Label(Sw.ER_EG.bes);
	...
	private Label lblRollladenOg = new Label(Sw.ER_OG.bes);
	...
	private Label lblRollladenDg = new Label(Sw.ER_DG.bes);
	...
	public boolean[] holeIsSelectedFuerCheckboxen() {
		return new boolean[] { chckBxSchiebetuerEg.isSelected(), chckBxSchiebetuerDg.isSelected(),
				chckBxEinbruchschutz.isSelected(), chckBxVorbereitungEg.isSelected(), chckBxVorbereitungOg.isSelected(),
				chckBxVorbereitungDg.isSelected(), chckBxRollladenEg.isSelected(), chckBxRollladenOg.isSelected(),
	...
	/*
	 * @Override public int[][] spinnerZu2DIntArray() { return null; }
	 */
	...
	protected void berechneUndZeigePreisSonderwuensche() {
		...
		if (chckBxSchiebetuerEg.isSelected())
			preis += Sw.STUEREN_TERRASSE.preis;
		if (chckBxSchiebetuerDg.isSelected())
			preis += Sw.STUEREN_DACHTERRASSE.preis;
		if (chckBxEinbruchschutz.isSelected())
			preis += Sw.EBS_HAUSTUER.preis;

		if (chckBxVorbereitungEg.isSelected())
			preis += Sw.VEAR_EG.preis;
		if (chckBxVorbereitungOg.isSelected())
			preis += Sw.VEAR_OG.preis;
		if (chckBxVorbereitungDg.isSelected())
			preis += Sw.VEAR_DG.preis;

		if (chckBxRollladenEg.isSelected())
			preis += Sw.ER_EG.preis;
		if (chckBxRollladenOg.isSelected())
			preis += Sw.ER_OG.preis;
		if (chckBxRollladenDg.isSelected())
			preis += Sw.ER_DG.preis;
	...
	/**
	 * Wird von BasisView-Button "Speichern" aufgerufen. Übergibt die Auswahl zum
	 * Speichern an Control.
	 */
	@Override
	protected void speichereSonderwuensche() {
		// Speichere Sonderwünsche (Prüfung in Control, da das Feld kundeModel private
		// ist)
	...
	protected void exportiereSonderwuenscheAlsCsv() {
		this.fensterControl.exportiereSonderwuenscheAlsCsv(checkboxenZuAnzahlSonderwuensche());
	}

	public void zeigeInfo(String header, String text) {
		if (Platform.isFxApplicationThread()) {
			...
			System.out.println("Info (ohne FX-UI): " + header + " - " + text);
		}
	}

	public void zeigeFehler(String header, String text) {
		if (Platform.isFxApplicationThread()) {
			...
			System.out.println("Fehler (ohne FX-UI): " + header + " - " + text);
		}
	}
