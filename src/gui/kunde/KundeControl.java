...
public class KundeControl {
	...
	/*
	 * Erstellen, falls nicht vorhanden, ein Control-Objekt. Das View wird sichtbar
	 * gemacht.
	 */
	...
	/**
	 * Lädt den Kunden zur angegebenen Hausnummer aus dem Model und zeigt ihn in der
	 * View an. Falls ein Datenbankfehler auftritt, wird eine Fehlermeldung in der
	 * View angezeigt.
	 *
	 * @param hausnummer die Hausnummer / Plannummer des Kunden, der geladen werden
	 *                   soll
	 */
	...
	public void exportiereAlleSonderwuenscheAlsCsv() {
		...
		try {
			kundeModel.exportiereAlleSonderwuenscheAlsCsv();
			kundeView.zeigeErfolgsmeldung("Export erfolgreich", "Sämtliche Sonderwünsche wurden als CSV exportiert.");
		} catch (IllegalStateException e) {
			kundeView.zeigeFehlermeldung("Ungültige Auswahl", e.getMessage());
		} catch (Exception e) {
			e.printStackTrace();
			kundeView.zeigeFehlermeldung("Fehler", "CSV-Export fehlgeschlagen.");
		}
	}
